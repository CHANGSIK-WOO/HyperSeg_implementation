...
from hyperseg.eval.eval_dataset.eval_datasets import VQA_Dataset_MMB
...
class DataArguments:
    ...
    eval_dataset: Optional[str] = field(default="mmb")
...
def evaluation():
    ...
    eval_dataset = VQA_Dataset_MMB(json_path=data_args.json_path, tokenizer=tokenizer, clip_image_processor=clip_image_processor,
                                                                    data_args=data_args)
    ...
    with torch.no_grad():
        for k, inputs in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
            idx = int(inputs["question_id"])
            cur_prompt = inputs["text"]

            options = inputs['options']
            cur_option_char = inputs['cur_option_char']
