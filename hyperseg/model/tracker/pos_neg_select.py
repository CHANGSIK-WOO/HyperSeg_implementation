...
from hyperseg.model.tracker.box_ops import box_cxcywh_to_xyxy, generalized_box_iou

def select_pos_neg(ref_box, all_indices, targets, embed_head, hs_key, hs_ref, ref_cls, det_targets=None, detach_reid=False,
    ...
    # indices(0): [12, 5, 78, 23] indices(1): [2, 1, 3, 0]

    for bz_i, (v, indices) in enumerate(zip(targets,all_indices)):
        num_insts = len(v["labels"]) 
        # tgt_valid = v["valid"].reshape(num_insts)
        tgt_bbox = v["boxes"].reshape(num_insts,4) 
        tgt_labels = v["labels"] # v["positive_map"]
        # tgt_valid = tgt_valid[:,1]    
        ref_box_bz = ref_box[bz_i] # (num_query, 4)
        ref_cls_bz = ref_cls[bz_i].softmax(-1) # (num_query, num_class)
        tgt_valid = v["valid"] # if obj exist in cur image
               
        contrastive_pos = get_pos_idx(ref_box_bz, ref_cls_bz, tgt_bbox, tgt_labels, tgt_valid)

        # get sorted indices
        _, sorted_indx = torch.sort(indices[1])

        # 提取排序后的indices_0部分
        query_indices = indices[0][sorted_indx]

        # loop over gts in one image
        for inst_i, (valid, matched_query_id) in enumerate(zip(tgt_valid, query_indices)):
    ...
    with torch.no_grad():  
        ...
        if False in valid:
            ...
            if valid.sum()>0:
                ...
                indices_batchi_neg_s = dynamic_k_matching(cost, pair_wise_ious, int(valid.sum()),30)
            valid_idx = 0
            ...
            if valid.sum()>0:
                ...
                indices_batchi_neg = dynamic_k_matching(cost, pair_wise_ious, bz_gtboxs.shape[0],30)
            else:
